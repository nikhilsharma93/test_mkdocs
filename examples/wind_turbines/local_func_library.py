...
def model_predict_func(model, img):
    # This is just a helper function, not a Component function
    pred_out = model.predict(img)
    return pred_out


def model_pred_wrapper(batch_imgs, break_tilesize, pad, unpadded_tilesize, model=5):
    batch_proba = None
    ...
    for batch_iter, img in enumerate(batch_imgs):
        for tile, src, dest in generate_tiles_tight(img, tilesize=break_tilesize, pad=pad):
            ...
            if batch_proba is None:  # Initialize a proba size
                batch_proba = np.empty((batch_size, int(unpadded_tilesize/16), int(unpadded_tilesize/16)) + \
                                        proba.shape[3:], dtype='float32')
    ...
    return dict(batch_proba=batch_proba)
...
def postprocess(batch_imgs, batch_ids, upload_dtype, break_tilesize, pad, unpadded_tilesize,
                batch_meta, batch_keys, batch_epsg):
    """Dummy."""
    image_ids = []
    upload_kwargs = []
    batch_size = batch_imgs.shape[0]

    batch_proba = batch_imgs
    for batch_iter, img in enumerate(batch_imgs):
        image_id = 'a_b.tif'
        image_ids.append(image_id)
        current_upload_kwargs = {'wkt_srs': batch_meta[batch_iter]['coordinateSystem']['wkt'],
                                 'geotrans': batch_meta[batch_iter]['geoTransform'],
                                 'raster_meta': batch_meta[batch_iter]}
